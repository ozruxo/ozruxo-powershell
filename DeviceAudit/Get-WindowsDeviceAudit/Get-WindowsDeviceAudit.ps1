...
.PARAMETER SkipEventLogs
    Specify not to also acquire the event logs. Since these take the longest to get and not always wanted.

.EXAMPLE
    Get-WindowsDeviceAudit -ComputerNames <ComputerName>

.EXAMPLE
    Get-WindowsDeviceAudit -ComputerNames <ComputerName>, <ComputerName>

.EXAMPLE
    Get-WindowsDeviceAudit -ComputerNames <ComputerName> -CreateCSVOnly

.EXAMPLE
    Get-WindowsDeviceAudit -ComputerNames <ComputerName> -CreateExcelFromCSVOnly
...
function Get-WindowsDeviceAudit {
    ...
    Begin{
        ...
        #region INITIAL VARIABLES
            ...
            $LogJobsDirectory  = "$RootLetter`:\logs\SystemAusitJobLogs"
            $LocalTempDirPath  = "$RootLetter`:\TMP"
            $RemoteRoot        = "$RootLetter$"
        ...
        #region LOCAL LOGGING AND DIRECTORY CREATION
            ...
            if(-not(Test-Path "$LogExcelDirectory")){
                ...
                Write-Verbose "Creating Directory on Desktop"
                New-Item -Path $LogExcelDirectory -ItemType Directory | Out-Null  
            }
            
            if(-not(Test-Path "$LogJobsDirectory")){
                
                Write-Verbose "Creating directory for logging on the root drive: $LogJobsDirectory"
        ...
        #region SWITCH PARAMETERS
            ...
            if ($CreateExcelFromCSVOnly -and (-not($CreateCSVOnly))){
                ...
                Write-Verbose "Create Excel from CSV only"
                Write-Host "`0"
    ...
    Process {
                ...
                function Get-TheGoods{
                    ...
                    #region GATHER SYSTEM PROPERTIES
                        ...
                        #region VARIABLES AND LOGGING
                            ...
                            $LogDate      = Get-Date -Format yyyy-MM-dd_hh-mm-ss
                            $RootLetter   = (Get-PSDrive | Where-Object {$_.Provider.Name -eq 'FileSystem'}).Root[0].split(':')[0]
                            $LogDirectory = "$RootLetter`:\Logs\SystemAudit\$env:COMPUTERNAME"
        ...
        foreach ($ComputerName in $ComputerNames){
            ...
            # Determine if computer is local
            if ($ComputerName -eq $env:COMPUTERNAME){

                $LocalDevice = $true
                Write-Verbose "Running against local device: $env:COMPUTERNAME"
            }

            # When device is not local device
            if ($LocalDevice -eq $false){
                
                if (-not(Test-Path "\\$ComputerName\$RemoteRoot\Windows") -and $false -eq $CreateExcelFromCSVPath){
                
                    # Append variable
                    $NoConnectionComputer.Add($ComputerName) | Out-Null
                    Write-Verbose "Could not connect to device: $ComputerName"
                    continue
                }
                Write-Verbose "Running against remote device: $ComputerName"
            }
            ...
            # Start Job
            Start-Job -Name $ComputerName -ArgumentList $ComputerName,$LogDate,$LogExcelDirectory,$CreateCSVOnly,$CreateExcelFromCSVPath,$LocalDevice,$LocalTempDirPath,$RemoteRoot -InitializationScript $JobScript -ScriptBlock {
                ...
                if ($False -eq $CreateExcelFromCSVPath){
                    ...
                    if ($LocalDevice){
                        ...
                        Write-Output "Starting job on local host"
                        Get-TheGoods
                    }
                    else {
                    
                        Write-Output "Starting job on remote host: $ComputerName"
                        Invoke-Command `
                        -ComputerName $ComputerName `
                        -ScriptBlock ${Function:Get-TheGoods}
                    }
                }

                #region CREATE TMP AND COPY CSV'S FROM REMOTE HOST UNLESS LOCAL
                    ...
                    if($false -eq $CreateExcelFromCSVPath){
                        ...
                        if(-not(Test-Path $TemporaryDirectoryPath)){
                            ...
                            Write-Output "Creating TMP directory"
                        }
                        if (!$LocalDevice){
                        
                            
                            Write-Output "Copying files from remote system to local system"
                            Copy-Item -Path "\\$ComputerName\$RemoteRoot\Logs\SystemAudit\$ComputerName\" -Destination $TemporaryDirectoryPath -Force -Recurse
                            if ($CreateCSVOnly){
                            ...
                            Write-Output "Moving files created on local host to TMP directory"
                            Move-Item -Path "$(($RemoteRoot).split('$')[0]):\Logs\SystemAudit\$ComputerName" -Destination $TemporaryDirectoryPath -Force
                            if ($CreateCSVOnly){
                                
                                Write-Output "Only CSV files needed. Stoping script"
                                Exit
                            }
                        }
                    }
                        ...
                        Write-Host "Creating Excel from specified CSV path"
                    }
                ...
                #region CREATE VARIABLES FOR EXCEL
                    ...
                    Write-Output "CSV'c imported to variables"
                ...
                $Excel.Quit()
                Write-Output "Excel file saved"
                
            } | Out-Null
            Write-Verbose "Job started for: $ComputerName"
        }
    }
